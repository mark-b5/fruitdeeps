...
export class Dps {
    ...
    setHitDist() {
        ...
        hitStore.store([1], acc / (max + 1));
        hitStore.store([1], acc / (max + 1));
        for (var dmg = 2; dmg <= max; dmg++) {  // Loop from 2 to max hit
        ...
        this.calcs.hitStore = hitStore;
        */
           // Store hit value 1 twice

        
        let altHitStore = new HitFreqStore();
        /* Update the clamped hit mechanics.
        In principle, just store the hit "1", twice, then do the normal thing.
        Previous lines:  
        altHitStore.store([0], 1 - altAcc); */
        
        altHitStore.store([0], 1 - altAcc);
        altHitStore.store([1], altAcc / (max + 1)); //entry one
        altHitStore.store([1], altAcc / (max + 1)); //entry two
        //for (var dmg = 0; dmg <= max; dmg++) {
        for (var dmg = 2; dmg <= max; dmg++) {  // Loop from 2 to max hit
        ...
        // hitStore can be modified from applySpecials so set hitDist after
    ...
    applySpecials() {
        ...
        if (this.calcs.flags.includes("Tonalztics of ralos")){ // add ralos
            this.calcs = specs.ralos()
        }
