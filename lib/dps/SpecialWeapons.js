...
export class SpecialWeapons {
    ...
    ralos() { 
        ...
        const newMax = Math.floor(baseMax * 0.75)
        const specMax = dps.maxHit

        // 
        let hitStore = new HitFreqStore();
        hitStore.store([0], 1-acc);
        hitStore.store([1], acc / (newMax + 1))
        hitStore.store([1], acc / (newMax + 1))

        for (var dmg = 2; dmg <= newMax; dmg++) {  // Loop from 2 to max hit
            hitStore.store([dmg], acc / (newMax + 1));
        }
    ...
    scytheDep() { //previous scythe mechanics
        const dps = this.calcs
        const acc = this.calcs.accuracy
        ...
        for (let hit1 = 0; hit1 <= m1; hit1++) {
            ...
            for (let hit2 = 0; hit2 <= m2; hit2++) {
                ...
                for (let hit3 = 0; hit3 <= m3; hit3++) {
                    ...
                    // Store the combination of hits and their probability
                    scytheHitStore.store([hit1, hit2, hit3], p1 * p2 * p3);
                }
            }
        }
