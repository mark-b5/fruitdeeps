...
export class SpecialAttacks {
    ...
    elder() {
        const dps = this.calcs
        const max = dps.maxHit
        const acc =  dps.accuracy * 1.25;
        const def = this.state.monster.stats.def

        let elderHitStore = new HitFreqStore();
        elderHitStore.store([0], 1 - acc);
        elderHitStore.store([1], acc / (max+ 1 ));
        elderHitStore.store([1], acc / (max + 1));
        ...
        eDefReduction += Math.trunc(def * 3.5 / 10) * acc

        for (let dmg = 2; dmg <= max; dmg++) {
            elderHitStore.store([dmg], acc / (max + 1))
        }

        dps.hitStore = elderHitStore
        dps.eDefReduction += eDefReduction
        dps.maxList = [max]
        dps.maxHit = max
        return dps;
    ...
    ralos() {
            ...
            const max = dps.maxHit;
            const acc = dps.accuracy;
            let monsterDef = this.state.monster.stats.def;
            const monsterMage = this.state.monster.stats.magic;
        
            let ralosHitStore = new HitFreqStore();
            let eDefReduction = 0;
            let eDefReduction1 = 0;
            let eDefReduction2 = 0;
        
            // First hit
            ralosHitStore.store([0], 1 - acc);
            ralosHitStore.store([1], acc / (max+1));
            ralosHitStore.store([1], acc / (max+1)); 

            for (let dmg1 = 2; dmg1 <= max; dmg1++) {
                const hit1Chance = acc / (max + 1);
                ralosHitStore.store([dmg1], hit1Chance);

                // Defense reduction from first hit
                eDefReduction1 = acc * monsterMage * 0.1; // need to rewrite accuracy roll for second one
                let monsterTempDef = Math.trunc(monsterDef - eDefReduction1)
                let npcRoll = accCalc.generalFormula(monsterTempDef + 9, this.state.monster.stats.drange);
                let acc2 = accCalc.compareRolls(this.calcs.playerRoll, npcRoll)
                eDefReduction2 = acc2 * monsterMage * 0.1; 
                ralosHitStore.store([0], 1 - acc);
                ralosHitStore.store([1], acc / (max+1));
                ralosHitStore.store([1], acc / (max+1)); 
                for (let dmg2 = 2; dmg2 <= max; dmg2++) {
                    const hit2Chance = acc2 / (max + 1);
                    ralosHitStore.store([dmg2], hit2Chance);
                }
            }
                eDefReduction = eDefReduction1; // Math.trunc(eDefReduction1 + eDefReduction2);

            dps.hitStore = ralosHitStore;
            dps.eDefReduction += eDefReduction;
            dps.maxList = [max, max];
            dps.maxHit = max;
            return dps;
